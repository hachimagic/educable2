...
async function fetchQuestionFromOpenAI(topic: string) {
		...
		let correctOptionKey = lines.find((line: string) => line.startsWith("Answer:"))?.split(':')[1]?.trim() || "1";
		if (!["1", "2", "3", "4"].includes(correctOptionKey)) correctOptionKey = "1";
		const solution = lines.find((line: string) => line.startsWith("Explanation:"))?.split(':')[1]?.trim() || "No explanation provided.";
